 ...
 */
package org.openmrs.module.insurance.dao;
 ...
 */
public class OrganizationDaoTest extends BaseModuleContextSensitiveTest {
	...
	public void saveItem_shouldSaveAllPropertiesInDb() {
		//Given
		/**
		 * Item item = new Item(); item.setDescription("some description");
		 * item.setOwner(userService.getUser(1)); //When dao.saveItem(item); //Let's clean up the
		 * cache to be sure getItemByUuid fetches from DB and not from cache Context.flushSession();
		 ...
		 * hasProperty("owner", is(item.getOwner()))); assertThat(savedItem,
